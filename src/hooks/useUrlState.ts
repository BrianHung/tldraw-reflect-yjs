...
import { Editor, TLPageId, debounce, react, useEditor, clamp, Vec } from "tldraw";
...
export const getViewportUrlQuery = (editor: Editor): UrlStateParams | null => {
  ...
  return {
    ...
    [PARAMS.p]: editor.getCurrentPageId(),
  };
};
...
export function useUrlState(onChangeUrl: (params: UrlStateParams) => void) {
  ...
  useEffect(() => {
    ...
    if (url.searchParams.has(PARAMS.p)) {
      const newPageId = "page:" + url.searchParams.get(PARAMS.p);
      if (newPageId) {
    ...
    if (url.searchParams.has(PARAMS.v)) {
      const newViewportRaw = url.searchParams.get(PARAMS.v);
      if (newViewportRaw) {
        try {
          ...
          editor.setCamera(new Vec(-x + (sw - w * zoom) / 2 / zoom, -y + (sh - h * zoom) / 2 / zoom, zoom), {
            immediate: true,
          });
        } catch (err) {
          console.error(err);
